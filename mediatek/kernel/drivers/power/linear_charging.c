...
void select_charging_curret(void)
{
  ...
  //<2014/04/07-Yuting Shih. Add the temperature level of battery check.
        ...
        //if(( g_BatTempLevel == TEMP_POS_45_TO_POS_55 ) &&
        //   ( BMT_status.bat_vol <= TEMP_CHECK_CHARGING_VOLTAGE ))
        if (TEMP_POS_DEC_CHARGE == g_batt_temp_status)
        {
            if( g_temp_CC_value > TEMP_CHECK_CHARGER_CURRENT )
                g_temp_CC_value = TEMP_CHECK_CHARGER_CURRENT;
        }
  #endif
    ...
    else
    {
        ...
        {    
			...
			/* Set CV */
					...
					cv_voltage = BATTERY_VOLT_04_350000_V;
				#else
                    //<2014/05/05-tedwu, change CV for LO2. Could reach 4.202V
                    #if defined( ARIMA_LO2_HW )
		                cv_voltage = BATTERY_VOLT_04_187500_V; //set to this, could reach 4.202V
		            #else
		                cv_voltage = BATTERY_VOLT_04_200000_V;
                    #endif
                    //>2014/05/05-tedwu
...
{
//<2014/04/28-tedwu, [LO2] Disable 1 second discharge.
#if !defined(ARIMA_LO2_HW)
