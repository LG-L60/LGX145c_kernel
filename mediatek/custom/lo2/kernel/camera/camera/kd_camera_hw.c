...
static  int  lcm_carema_onoff = 0;
extern  void lcm_set_for_camera_on(void);
extern  void lcm_set_for_camera_off(void);
#endif
//>2014/05/30-Yuting Shih.


static void MainCameraDigtalPowerCtrl(kal_bool on){
//<20140222-tedwu, disabled due to use VGP3, not the external LDO.
/*
...
//<20140306-kyle chang, Add Sub-camera
    PK_DBG("kdCISModulePowerOn: 5EA OV7695\n");
//>20140306-kyle chang
    ...
    if (On) {
       ...
       if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K5EAYX_YUV,currSensorName)))//<2014/02/25-kyle chang, Change Main Camera>
       {
		   PK_DBG("[CAMERA SENSOR] kdCISModulePowerOn get in---S5K5EAYX_YUV sensorIdx:%d; pinSetIdx=%d\n",SensorIdx, pinSetIdx);
	   
		   if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name)) //MT6323_POWER_LDO_VCAM_IO=1.8V
		   {
		   ...
		   if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200,mode_name)) //MT6323_POWER_LDO_VGP3=1.2V
		   {
			   PK_DBG("[CAMERA SENSOR] Fail to enable digital Core power 1.2V\n");
			   //return -EIO;
			   goto _kdCISModulePowerOn_exit_;
		   }
	   
		   if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800,mode_name)) //MT6323_POWER_LDO_VCAMA=2.8V
		   {
			   PK_DBG("[CAMERA SENSOR] Fail to enable analog Pixel power 2.8V\n");
			   //return -EIO;
			   goto _kdCISModulePowerOn_exit_;
		   }
	   
	   	   /*
		   if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A2, VOL_2800,mode_name)) //MT6323_POWER_LDO_VCAM_AF
		   {
...
//<20140306-kyle chang, Add Sub-camera
       else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV2675_MIPI_YUV,currSensorName)))
       {  
		   PK_DBG("[CAMERA SENSOR] kdCISModulePowerOn get in---OV2675_MIPI_YUV sensorIdx:%d; pinSetIdx=%d\n",SensorIdx, pinSetIdx);
