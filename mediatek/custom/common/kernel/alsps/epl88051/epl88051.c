...
static int epl_psensor_enable(struct epl_priv *epl_data, int enable)
{
    ...
    ret = epl_I2C_Write(client,REG_6,W_SINGLE_BYTE,0x02, EPL_INT_DISABLE |  EPL_INT_CH1);
...
///add delta to set threshold
	APS_LOG("read ps raw data= %d,gRawData.dynaK=%d,gRawData.cal_delta=%d \n", gRawData.ps_raw, gRawData.dynaK, gRawData.cal_delta);
	if (gRawData.dynaK == 1)
	{
	    u16 dyna_threshold;
