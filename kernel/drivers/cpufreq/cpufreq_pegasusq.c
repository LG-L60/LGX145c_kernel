...
//#define PEGASUSQ_DEBUG

#if PEGASUSQ_DEBUG
#define dprintk(msg...) do { \
	printk(msg); \
	} while (0)
#define pr_debug(msg...) do { \
	printk(msg); \
	} while (0)
#else
#define dprintk(msg...)
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret) {
		dprintk(KERN_ERR "[HOTPLUG] already locked with smaller value %d < %d\n",
...
{
	int cpu;
	dprintk(KERN_ERR "CHECK %s rq %d.%02d freq %d [", which ? "up" : "down",
	       rq_avg / 100, rq_avg % 100, freq);
	for_each_online_cpu(cpu) {
		dprintk(KERN_ERR "(%d, %d), ", cpu, usage->load[cpu]);
	}
	dprintk(KERN_ERR "]\n");
}
...
static int check_up(void)
{
	...
	if (avg_freq >= up_freq && avg_rq > up_rq) {
		dprintk(KERN_ERR "[HOTPLUG IN] %s %d>=%d && %d>%d\n",
...
static int check_down(void)
{
	...
	if (avg_freq <= down_freq && avg_rq <= down_rq) {
		dprintk(KERN_ERR "[HOTPLUG OUT] %s %d<=%d && %d<%d\n",
