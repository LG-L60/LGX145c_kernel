...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	for_each_process(tsk) {
...
#if defined (MTK_LCA_RAM_OPTIMIZE)		
		// For KK CR ALPS01426325, ALPS01458034: walkaround CTS issue
		// if cached > 30MB, don't kill specific processes while its adj is 9
		...
		if (!strcmp(p->comm, "id.cts.verifier") && (REVERT_ADJ(oom_score_adj)==9) && (other_file > 30*256)) {
		    lowmem_print(1, "select but ignore '%s' (%d), oom_score_adj %d, oom_adj %d, size %d, to kill\n" \
		                    "cache %ldkB is below limit %ldkB",
			     p->comm, p->pid, oom_score_adj, REVERT_ADJ(oom_score_adj), tasksize,			     
			     other_file * (long)(PAGE_SIZE / 1024),
